...
Generative retrieval replaces the encode-and-search pipeline with a single sequence model that emits a target's identifier directly: the model's parameters become the index. DSI~\cite{tay2022dsi} casts document retrieval as text-to-identifier generation and trains with a joint indexing-and-retrieval objective. TIGER~\cite{rajput2023tiger} introduced Semantic IDs constructed via RQ-VAE over item embeddings; its headline finding---that structured codes substantially outperform arbitrary identifiers---is the result our SemID-condition comparison tests in the audio domain. Music-domain extensions include Text2Tracks~\cite{palumbo2025text2tracks} (text prompt to track), FusID~\cite{kim2026fusid} (multimodal Semantic IDs for playlist continuation), and large-scale catalog work at industry labs~\cite{mei2025semids}. All target recommendation or text-query settings; none addresses cover song identification, where the query and target are both audio and ground truth is unambiguous.
...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners, align=center, font=\footnotesize,
              inner sep=3.5pt, minimum width=0.74\columnwidth},
  arr/.style={-{Latex[length=1.8mm]}, semithick},
]
\node[box] (audio) {Query track audio \;(30\,s, 24\,kHz mono)};
\node[box, below=of audio] (mert) {MERT-v1-95M \;(frozen) $\rightarrow$ 768-d};
\node[box, below=of mert] (quant) {Quantizer: RQ-VAE \,/\, EnCodec \,/\, random};
\node[box, below=of quant] (sid) {Query Semantic ID $\langle c_1\,c_2\,c_3\rangle$};
\node[box, below=of sid] (t5) {T5-small \;(fine-tuned) + constrained beam search};
\node[box, below=of t5] (track) {Cover Semantic ID $\rightarrow$ track-ID lookup};
\draw[arr] (audio) -- (mert);
\draw[arr] (mert) -- (quant);
\draw[arr] (quant) -- (sid);
\draw[arr] (sid) -- (t5);
\draw[arr] (t5) -- (track);
\end{tikzpicture}
\caption{Generative retrieval pipeline. A frozen audio encoder and a quantizer map each track to a discrete Semantic ID; a fine-tuned T5 autoregressively generates the Semantic ID of a cover, which is resolved to a track. The model's parameters replace the embedding index.}
\label{fig:pipeline}
\end{figure}

\subsection{Task Formulation}
For each clique $\mathcal{C} = \{t_1, t_2, \ldots, t_n\}$ in the training corpus, we generate all ordered pairs $(t_i, t_j),\, i \neq j$. The model receives the Semantic ID of a query track $t_q$ and emits the discrete Semantic ID $\mathrm{SID}(t_j)$ of a cover (Fig.~\ref{fig:pipeline}). Retrieval is performed by constrained beam search; predicted SIDs are resolved to track IDs via a lookup table. A prediction is correct if the resolved track lies in the query's clique.
...
We use T5-small (60M parameters)~\cite{raffel2020t5}. We extend the tokenizer with $3K$ SemID tokens---one per (level, code) pair---and resize the model's input and output embedding matrices. The encoder input is the prefix \texttt{"retrieve cover:"} followed by the query's SemID token sequence; the decoder generates the target SemID. Training uses teacher forcing with cross-entropy loss.
...
\subsection{The Transductive Boundary: a Cold-Start Experiment}
Generative retrieval memorizes a fixed corpus---the model's parameters are the index. To measure this boundary directly, we re-ran all three conditions with an \emph{inductive} clique-level split: 487/104/105 train/val/test cliques, with test cliques entirely unseen during training (same training recipe otherwise). Table~\ref{tab:coldstart} reports the result.

\begin{table}[t]
\caption{Cold-start (inductive) retrieval on Discogs-VI: 105 test cliques entirely held out from training. The bi-encoder has no training phase; T5 columns are generative retrieval.}
\label{tab:coldstart}
\centering
\small
\begin{tabular}{lcccc}
...
\midrule
Bi-encoder (MERT)  & 15.0 & \textbf{0.193} & \textbf{0.146} & \textbf{0.308} \\
...
All three generative conditions score \textbf{exactly zero} on every retrieval metric. The models are not degenerate---they emit 29--59 distinct tracks across the 390 queries---but every generated Semantic ID belongs to a \emph{training} clique. The model cannot reach a clique it never indexed. The bi-encoder, having no training phase, retrieves held-out covers exactly as well as in-distribution ones.

The contrast with Table~\ref{tab:main_results} is the paper's central scoping result: \textbf{generative retrieval for CSI wins when the corpus is fixed and indexed (MRR 0.271 vs.\ 0.214) and fails completely on new songs (0.000)}. Replacing the search index with model parameters is the source of both the transductive gain and the inductive failure. For an open or growing catalog, the bi-encoder remains necessary; for a fixed catalog, generative retrieval is the stronger choice.

\subsection{Error Analysis: What the Model Retrieves When It Fails}
For the transductive MERT-RQ-VAE model we split the 2{,}324 test queries into hits (867) and misses (1{,}457) and measured MERT cosine similarity between the query and three references (Fig.~\ref{fig:error}):

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/error_analysis.pdf}
\caption{Distribution of MERT cosine similarity between a query and: a random track (floor), T5's top-1 prediction on a miss, and the query's true covers. T5's misses are acoustically plausible---and in fact \emph{more} similar to the query than the true covers.}
\label{fig:error}
\end{figure}

The query-to-true-cover similarity is 0.886; query-to-random is 0.869; and query-to-T5-miss is \textbf{0.900}. Two observations follow. First, the model degrades gracefully: a wrong prediction (0.900) is far closer to the query than a random track (0.869), so failures are plausible near-misses, not noise. Second, and more revealing, T5's misses are \emph{more} acoustically similar to the query than the true covers are. The model fails by retrieving acoustic near-duplicates. This is direct evidence that MERT-RQ-VAE Semantic IDs encode acoustic surface similarity, which only approximates compositional identity: covers involving heavy reinterpretation---substantial key, tempo, or instrumentation change---are acoustically distant from the query and are systematically missed. Closing this gap is the key open problem for audio-derived generative retrieval.

\section{Limitations and Future Work}
\textbf{Corpus scale.} The Discogs-VI subset is constrained by YouTube link rot and bot detection ($\sim$72\% loss from a 12k-track crawl). The 3{,}192-track corpus is two orders of magnitude smaller than the catalogs in TIGER-style recommendation work. Recovering bot-detected entries with authenticated cookies is straightforward and would multiply the eval set; we report the un-recovered subset for reproducibility. A larger corpus would also test whether the transductive advantage scales.

\textbf{The transductive constraint.} Our central negative result---zero cold-start performance---is the sharpest limitation. A generative retriever cannot serve a growing catalog without retraining or an explicit indexing objective that teaches new track identities. A DSI-style indexing task~\cite{tay2022dsi}, interleaving (audio$\rightarrow$own-SemID) examples with the retrieval objective, is the natural next step and could partially relax the constraint.

\textbf{Acoustic vs.\ compositional similarity.} The error analysis shows the model retrieves acoustic near-duplicates when it fails. MERT-RQ-VAE Semantic IDs inherit MERT's acoustic bias; covers with large key, tempo, or instrumentation change fall outside it. Semantic IDs built from explicitly compositional features (harmonic progression, melodic contour) could close this gap.

\textbf{Modeling choices.} The model trains on 30-second center clips, ignoring long-form structure, and uses only the SemID-to-SemID input mode; the continuous-MERT-to-SemID variant via a learned encoder projection is left to future work.
...
\bibitem{vijayakumar2018diverse} A. K. Vijayakumar et al., ``Diverse beam search: decoding diverse solutions from neural sequence models,'' in \emph{AAAI}, 2018.
\end{thebibliography}
